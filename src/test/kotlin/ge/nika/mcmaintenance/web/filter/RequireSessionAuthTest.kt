...
class RequireSessionAuthTest {
    ...
    fun `returns response for next handlers if valid session exists and adds users id header to request`() {
        ...
        val service = mockk<LogInService> {
            every { getSessionIfValid(any(), any()) } returns Session("aaa", "nika123", LocalDateTime.now())
        }
    ...
    @Test
    fun `returns forbidden status when session-id is provided but session is invalid`() {
        val service = mockk<LogInService> { every { getSessionIfValid(any(), any()) } returns null }
    ...
    fun `can decorate a routed handler`() {
        ...
        val service = mockk<LogInService> {
            every { getSessionIfValid("valid", any()) } returns Session("aaa", "nika123", LocalDateTime.now())
            every { getSessionIfValid("invalid", any()) } returns null
        }
